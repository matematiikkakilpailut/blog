...
Kaikki palapelin palaset on nyt koottu, ja ideana on soveltaa lausetta 3 väitteen todistamiseen, ehkä käyttäen inkluusio-ekskluusiota ja induktiota. Kyse on enää idean toteuttamisesta. Keksin mielestäni varsin sievän tavan toteuttaa idean, joka lähestyy ongelmaa toisesta näkökulmasta.

Olkoon $F$ polynomien $P_1, P_2, \ldots , P_n$ juurikuntien compositum. Edellisestä päättelystä ei ole vaikeaa nähdä, että $C$ on kanta $\mathbb{Q}$-vektoriavaruudelle $F$. Jokainen $F$:n alkio voidaan siis kirjoittaa muodossa
...
Todistus on triviaali: määritellään $f(\sqrt{a_i}) = s_ia_i$. Voimme vain laajentaa $f$:n olemaan isomorfismi $F \to F$. Se, miksi tämä toimii, johtuu siitä, että $[F : \mathbb{Q}] = 2^n$ - tästä seuraa yllä esitetty $C$:n alkioiden lineaarinen riippumattomuus.
...
Frobeniuksen lause todistaa väitteen.

Emme siis missään kohtaa käyttäneet lausetta 3, vaikkakin lauseeseen 3 tarvittavaa oletusta käytettiin. Tämä todistus on tietyssä mielessä oikea tapa ajatella lausetta 3 - lauseen ehto varmistaa, että polynomin $P_1P_2 \cdots P_n$ Galois'n ryhmä on vain $G_1 \times G_2 \times \cdots \times G_n$, missä $G_i$ on $P_i$:n Galois'n ryhmä. Tällöin väite seuraa Frobeniuksen lauseesta.
...
Olkoot $m, n \in \mathbb{Z}_+$, ja olkoon $d$ niiden syt. Olkoon vielä $a \in \mathbb{Z}$. Pätee:
